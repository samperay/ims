...
@app.get("/server/{hostname}", response_model=Server, status_code=200)
...
@app.post("/server", response_model=Server, status_code=201)
async def add_server_to_inventory(server: dict):
    server_database.get("servers").append(server)
    return server


@app.delete("/server/{hostname}", status_code=204)
...
@app.put("/server/{hostname}", response_model=Server, status_code=201)
